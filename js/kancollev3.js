...
if(!(isIE(5)||isIE(6)||isIE(7)||isIE(8)))
{
	...
	if(hour != null && minute != null)
	{
		...
		if(mtime > 0)
		{
			...
			else
			{
				...
				if(ld == 0)
				{
					htmlo.innerHTML = "<h4>距离抢号时间还有<br><br>" + seczero(lh) + "小时" + seczero(lmi) + "分" + seczero(ls) + "秒</h4>";
				}
				else
				{
					htmlo.innerHTML = "<h4>距离抢号时间还有<br><br>" + seczero(ld) + "日" + seczero(lh) + "小时" + seczero(lmi) + "分" + seczero(ls) + "秒</h4>";
				}
			}
...
function XMLRefresh()
{
	$().ready(function (e)
	{
		...
		{
			traceurl ="http://www.haoyuan.info/php/trace2.php?client=web&random="+nowtimesec();
			urladdr ="http://www.haoyuan.info/php/trace2.php?ajax=1&r=refresh&client=web&t=100&random=" + nowtimesec();
		}
		else
		{
...
function asyncxml()
{
	...
	if(location.hostname.indexOf("host")!=-1)
	{
		endurl ="http://www.haoyuan.info/twitter/index.php?e=end&random="+nowtimesec();
	}
	else
	{
...
function loopajax()
{
	var loop_request=new XMLHttpRequest();
	var req_addr=new String();
	if(location.hostname.indexOf("host") != -1)
	{
	req_addr = "http://www.haoyuan.info/php/trace2.php?ajax=1&r=refresh&client=web&t=100&random=" + nowtimesec();
	}
	else
	{
	req_addr = "http://" + location.hostname + "/php/trace2.php?ajax=1&r=refresh&client=web&t=100&random=" + nowtimesec();
	}
		loop_request.onreadystatechange = function ()
		{
			if(loop_request.readyState == 4 && loop_request.status == 200)
			{
				eval(loop_request.responseText);
				maincore(1);
				maintenance(maintenance_m,maintenance_d,maintenance_start_hour,maintenance_start_minute,maintenance_end_hour,maintenance_end_minute);
				loaded();
				setTimeout("loopajax()", 900000);
			}
			else if(loop_request.readyState == 4 && loop_request.status == 500)
			{
				try{
				eval(loop_request.responseText);
				maincore(1);
				maintenance(maintenance_m,maintenance_d,maintenance_start_hour,maintenance_start_minute,maintenance_end_hour,maintenance_end_minute);
				loaded();
				$("div#error-dialog").fadeIn(1000);
				setTimeout(function(){$("div#error-dialog").fadeOut(2000);},3000);
				setTimeout("loopajax()", 900000);
				}
				catch(e)
				{
		...
		loop_request.open("GET", req_addr, true);
		loop_request.send();
}